      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-cream-50/80 border-b border-grass-100">
            ...
            <tbody>
              {players.length === 0 ? (
                ...
                editingId === p.id ? (
                  <tr key={p.id} className="border-b border-grass-100 bg-cream-50/40">
                  ...
                  <tr key={p.id} className={`border-b border-grass-100/50 hover:bg-cream-50/40 ${!p.active ? 'opacity-50' : ''}`}>
                    ...
                    <td className="px-4 py-2">
                      <button
                        ...
                        className={`pill ${p.active ? 'bg-grass-100 text-ink-900' : 'bg-cream-50 text-ink-700/40'}`}
                      >
                        {p.active ? 'Active' : 'Inactive'}
                      </button>
                    </td>
                    <td className="px-4 py-2 text-right">
                      <button onClick={() => setEditingId(p.id)} className="p-1.5 hover:bg-grass-100 rounded" title="Edit">
                        <Edit3 className="w-4 h-4" />
                      </button>
                      <button onClick={() => deletePlayer(p.id)} className="p-1.5 hover:bg-grass-100 rounded text-grass-700" title="Delete">
