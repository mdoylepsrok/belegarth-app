...
export default function Stats() {
  ...
  if (loading) return <p className="text-ink-700/60">Loading stats...</p>;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <label className="text-sm text-ink-700/60">Sort by:</label>
      ...
      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-cream-50/80 border-b border-grass-100 text-ink-700/60 text-left">
              <tr>
            ...
            <tbody>
              {sorted.length === 0 ? (
                <tr><td colSpan={9} className="px-4 py-8 text-center text-ink-700/50">
                  No stats yet. Play some battles!
                </td></tr>
              ) : sorted.map((p, idx) => (
                <tr key={p.id} className="border-b border-grass-100/50 hover:bg-cream-50/40">
                  <td className="px-3 py-2 text-ink-700/40">
                  ...
                  <td className="px-3 py-2 text-right text-ink-900">{p.total_kills}</td>
                  <td className="px-3 py-2 text-right text-ink-700/60">{p.total_deaths}</td>
                  ...
                  <td className="px-3 py-2 text-right text-ink-700/60">{p.sessions_attended}</td>
                </tr>
              ))}
            </tbody>
          </table>
