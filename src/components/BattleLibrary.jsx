...
export default function BattleLibrary() {
  ...
  if (loading) return <p className="text-parchment-100/60">Loading library...</p>;
          ...
          editingId === g.id ? (
            ...
            <div key={g.id} className={`card p-4 ${!g.in_pool ? 'opacity-50' : ''}`}>
              <div className="flex items-start justify-between gap-2 mb-2">
                <h3 className="font-display text-lg text-blood-400">{g.name}</h3>
                <div className="flex gap-1">
                  <button onClick={() => setEditingId(g.id)} className="p-1.5 hover:bg-forest-700 rounded" title="Edit">
                    <Edit3 className="w-4 h-4" />
                  </button>
                  <button onClick={() => deleteGame(g.id)} className="p-1.5 hover:bg-forest-700 rounded text-blood-400" title="Delete">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              </div>
              <div className="flex gap-2 text-xs mb-2 flex-wrap">
                <span className="pill bg-forest-700 text-parchment-100/80">{g.team_count} teams</span>
                <span className="pill bg-forest-700 text-parchment-100/80">Min {g.min_players}</span>
                <button
                  onClick={() => togglePool(g)}
                  className={`pill ${g.in_pool ? 'bg-blood-500 text-parchment-50' : 'bg-forest-900 text-parchment-100/40'}`}
                  title="Click to toggle inclusion in random pool"
                >
                  {g.in_pool ? 'In pool' : 'Excluded'}
                </button>
              </div>
              {g.description && <p className="text-sm text-parchment-100/80 mb-1">{g.description}</p>}
              {g.rules && <p className="text-xs text-parchment-100/60 italic whitespace-pre-line">{g.rules}</p>}
            </div>
          )
        ))}
        {games.length === 0 && (
          <div className="md:col-span-2 card p-8 text-center text-parchment-100/50">
