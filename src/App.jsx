...
import { IdentityPickerModal, useIdentity } from './lib/identity.jsx';
...
export default function App() {
  ...
  const { ready: identityReady, player, promptIdentity } = useIdentity();

  useEffect(() => { setConfigured(isConfigured()); }, []);

  if (!authReady || !identityReady) return null;
        ...
        <footer className="border-t border-grass-100 py-4 bg-white/40">
          <div className="max-w-6xl mx-auto px-4 flex items-center justify-between text-xs text-ink-700/50 flex-wrap gap-2">
            ...
            <div className="flex items-center gap-3">
              {player ? (
                <button onClick={promptIdentity} className="hover:text-ink-900">
                  I'm {player.belegarth_name || player.name}
                </button>
              ) : (
                <button onClick={promptIdentity} className="hover:text-ink-900">
                  Identify yourself
